...
void check(T result, char const *const func, const char *const file, int const line)
{
    if (result)
    {
        fprintf(stdout, "CUDA error at %s:%d code=%d(%s) \"%s\" \n",
...
inline void __getLastCudaError(const char *errorMessage, const char *file, const int line)
{
    ...
    if (cudaSuccess != err)
    {
        fprintf(stdout, "%s(%i) : getLastCudaError() CUDA error : %s : (%d) %s.\n",
...
inline int gpuDeviceInit(int devID)
{
    ...
    {
        fprintf(stdout, "Error: device is running in <Compute Mode Prohibited>, no threads can use ::cudaSetDevice().\n");
        return -1;
    }
...
inline int gpuGetMaxGflopsDeviceId()
{
    ...
    if (device_count == 0)
    {
        fprintf(stdout, "gpuGetMaxGflopsDeviceId() CUDA error: no devices supporting CUDA.\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
    	fprintf(stdout, "gpuGetMaxGflopsDeviceId() CUDA error: all devices have compute mode prohibited.\n");
    	exit(EXIT_FAILURE);
    }
