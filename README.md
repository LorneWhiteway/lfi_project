...
2. This step usually done by Niall: in the runs directory create a text file containing cosmological parameters for the runs. This file should have one header row, followed by one row per simulation. There should be seven columns (comma delimited): Omega_m, sigma_8, w0, wa, Omega_b, little_h, n_s, and m_nu (the latter in eV). The file should have a name that matches the pattern `params*.txt`. (Old versions of these files had only seven columns as the wa column was missing; this format is deprecated.)
