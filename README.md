...
- Run `./cm_locust.sh`. You will need to type `y` to confirm the build directory name.
...
The input argument 'use_concept_string' should be either 'concept' or 'noconcept' to determine whether pkdgrav3 should get its transfer function and other cosmological information from should use a CONCEPT-generated hdf5 file or from a transfer function file (plus cosmological parameters specified in the control file).
...
## Google documents with summaries of which runs have been done
...
2. This step usually done by Niall: in the runs directory create a text file containing cosmological parameters for the runs. This file should have one header row, followed by one row per simulation. There should be eight columns (comma delimited): Omega_m, sigma_8, w0, wa, Omega_b, little_h, n_s, and m_nu (the latter in eV). The file should have a name that matches the pattern `params*.txt`. (Old versions of these files had only seven columns as the wa column was missing; this format is deprecated.)
