...
4. Update the data file `runs_directory_data.txt` in the scripts directory. This is a comma-delimited text file with one row per runs directory. Each row should have the letter denoting the runs, followed by an integer to be used as an offset for the random seeds for these runs, followed by the pattern for the naming convention for the hdf5 files (see step 3), with the run number string replaced by `{}`. For the random seed offset use the number from the previous set of runs, plus the number of runs in that previous set. Example line: `U,1394,class_processed_gs2_batch2_{}.hdf5`. Note that set `T` used a random seed offset of 882, and set `T` had 512 runs.
...
7. Run the script `create_input_files.py` (found in the `scripts` directory) with the command line parameters runs_letter and the ru directories to be created. Example: `./create_input_files.py U 1-100'.
