...
void mdlLaunch(int argc,char **argv,void (*fcnMaster)(MDL,void *),void * (*fcnWorkerInit)(MDL),void (*fcnWorkerDone)(MDL,void *)) {
	...
	printf("===================== mdl->base.nCores = %d; set from _SC_NPROCESSORS_CONF\n", mdl->base.nCores);
#endif
    ...
    */
    ...
    if(argv) {
        ...
        while (argv[i]) {
            if (!strcmp(argv[i], "-sz") && !bThreads) {
				...
				printf("===================== mdl->base.nCores = %d; set from -sz\n", mdl->base.nCores);
    ...
    if (!bThreads) {
        if ( (p=getenv("SLURM_CPUS_PER_TASK")) != NULL ) {
            mdl->base.nCores = atoi(p);
			printf("===================== mdl->base.nCores = %d; set from SLURM_CPUS_PER_TASK\n", mdl->base.nCores);
		}
        else if ( (p=getenv("OMP_NUM_THREADS")) != NULL ) {
            mdl->base.nCores = atoi(p);
			printf("===================== mdl->base.nCores = %d; set from OMP_NUM_THREADS\n", mdl->base.nCores);
		}
	}
    assert(mdl->base.nCores>0);
	
	printf("===================== mdl->base.nCores = %d\n", mdl->base.nCores);
